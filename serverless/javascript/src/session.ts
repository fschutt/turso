...
export class Session {
   ...
   * The server applies its is_stateless() check and returns baton: null when
   * the stream can be closed; we just trust that and store whatever comes back.
   */
   ...
   *
   * Uses the /v3/pipeline endpoint so the server runs its is_stateless() check
   ...
   * is_stateless() auto-close check — so the leftover baton will be cleaned
   * up by the next pipeline call when the server re-evaluates stream state.
   */
